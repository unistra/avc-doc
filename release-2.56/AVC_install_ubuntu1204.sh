...
apt-get update
apt-get upgrade
apt-get install postgresql tomcat6 apache2 vsftpd zip unzip mailutils libpg-java libcommons-fileupload-java libcommons-io-java libcommons-lang-java libcommons-beanutils-java libcommons-collections3-java libcommons-logging-java ant git liblog4j1.2-java openjdk-6-jdk
#paquets pour l'encodage
apt-get install vorbis-tools lame ffmpeg libavcodec-extra-53 eyeD3 ffmpeg2theora yamdi zip unzip python2.7 python-reportlab python-psycopg2 atomicparsley bc
...
git clone https://github.com/unistra/avc-server.git univ-r_av && cd univ-r_av && git checkout release-2.56 && cd ..
echo "------------------------------------"
